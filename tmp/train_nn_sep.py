...
class MyLightningModule(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        attention_mask3 = x["attention_mask3"]
        attention_mask4 = x["attention_mask4"]
        token_type_ids1 = x["token_type_ids1"]
        token_type_ids2 = x["token_type_ids2"]
        token_type_ids3 = x["token_type_ids3"]
        token_type_ids4 = x["token_type_ids4"]
        features = x["features"]
        output = self.backbone(
            ...
            token_type_ids4=token_type_ids4,
            features=features,
        )
        return output
    ...
    def training_step(self, batch, batch_idx):
        ...
        token_type_ids3 = batch["token_type_ids3"]
        token_type_ids4 = batch["token_type_ids4"]
...
class TextDataset(Dataset):
    def __init__(
        ...
        self.encoded3 = tokenizer.batch_encode_plus(
            self.df[f"{text_col}_1"].str[:32].fillna("").tolist(),
            padding="max_length",
            max_length=max_len,
            truncation=True,
            return_attention_mask=True,
        ...
        self.encoded4 = tokenizer.batch_encode_plus(
            self.df[f"{text_col}_2"].str[:32].fillna("").tolist(),
            padding="max_length",
            max_length=max_len,
            truncation=True,
            return_attention_mask=True,
    ...
    def __getitem__(self, index):
        ...
        input_ids3 = torch.tensor(self.encoded3["input_ids"][index])
        attention_mask3 = torch.tensor(self.encoded3["attention_mask"][index])
        token_type_ids3 = torch.tensor(self.encoded3["token_type_ids"][index])
        input_ids4 = torch.tensor(self.encoded4["input_ids"][index])
        attention_mask4 = torch.tensor(self.encoded4["attention_mask"][index])
        token_type_ids4 = torch.tensor(self.encoded4["token_type_ids"][index])
...
class MyModel(nn.Module):
    def __init__(
        ...
        self.net3 = AutoModel.from_pretrained(model_path, config=model_config)
        self.net4 = AutoModel.from_pretrained(model_path, config=model_config)
    ...
    def forward(
        ...
        if self.custom_header == "max_pool":
            ...
            sequence_output3, _ = outputs3["last_hidden_state"].max(1)
            sequence_output4, _ = outputs4["last_hidden_state"].max(1)
            ...
            sequence_output3 = torch.cat(
                [outputs3["hidden_states"][-1 * i][:, 0] for i in range(1, 4 + 1)],
                dim=1,
            )
            sequence_output4 = torch.cat(
                [outputs4["hidden_states"][-1 * i][:, 0] for i in range(1, 4 + 1)],
                dim=1,
            )
        sequence_output = torch.cat(
            [
                ...
                torch.abs(sequence_output3 - sequence_output4),
                sequence_output3 * sequence_output4,
                features,
            ],
            dim=1,
        )
